  ...
  Plugin.prototype = {
    ...
    setup: function(table,tableSortContainer) {
      ...
      function scrollActive(scrollPos) {
        ...
        table.css('right',(tablePercentage*-1)+'%');
        scrollbar.css('right',scrollbarPercentage+'%');
      }
      ...
      function scrollPassive() {
        ...
        if ((parseFloat(scrollbarStyles.right)+parseFloat(scrollbarStyles.width)) > 100) {
          var right            = (parseFloat(scrollbarStyles.right)+parseFloat(scrollbarStyles.width))-100;
          var percentage      = (parseFloat(scrollbarStyles.right)-right);

          scrollActive(Math.round((percentage*scrolltrack.width())/100));
        } else {
          var percentage = parseFloat((parseFloat(scrollbarStyles.right)*scrolltrack.width())/100);
      ...
      function setUpScroll() {
        ...
        function scrollEvent() {
          ...
          scrolltrack.on('mousedown',function (e) {
            initPos      = scrollbar.offset().right-scrolltrack.offset().right;
            initMousePos = e.pageX-scrolltrack.offset().right;
            scroll.addClass('scrollbar-active');
            $('html').on('mousemove',function (e) {
              var scrollbarWidth   = scrollbar.width();
              var scrolltrackWidth = scrolltrack.width();
              newMousePos          = e.pageX-scrolltrack.offset().right;
    ...
    search: function (table, tableSortContainer) {
        ...
        tr.each(function () {
          ...
          if (matchText.match(match) !== null) {
            el.find(':not(:has(*))').each(function () {
              var target = $(this);
              replaced   = target.text().replace(match,function (m,e) {
                return m;
              });
      ...
      if (table.hasClass('table-sort-search')) {
        ...
        var search        = $('<div class="table-sort-search-container"><input class="table-sort-search-input" type="text" placeholder="جستجو"></div>');
